...
/**
 * An enum of predefined, fixed cards that can be played in the game. Each enum constant holds a {@link GameCard} instance.
 *
 * @author Lunkle
 */
@Derializable
public enum FixedCards {
	...
	UNSTABLE_TELEPORT(new GameCard(
			"Unstable Teleport",
			"teleport",
	...
	ELECTROSTATIC_ZAPPER(new GameCard(
	...
	FREEZE(new GameCard(
			...
			2,
			20,
			null, // linked below in static initializer
			new TargetingInfo(CARD_PLAYER, new RangeCondition(5))
	)),
	VENOMOUS_STRIKE(new GameCard(
			"Venomous Strike",
			"venomous_strike",
	...
	SPIDERLING(new GameCard(
			...
			2,
			20,
			null, // dynamically linked below in static initializer
			new TargetingInfo(HEXAGON, new RangeCondition(1), new EmptyCondition(new ActorsOnTilesQuery(new TargetQuery<>())))
	)),
	DEATHBLOOM(new GameCard(
			"Deathbloom",
			"deathbloom",
	...
	DEBILITATING_FEAR(new GameCard(
			"Debilitating Fear",
			"fear",
	...
	static {
		FREEZE.card.expression(and(
				addCardToStack(ICE_CUBE.card(), new TargetQuery<>()),
				new SpawnParticlesExpression(
		...
		SPIDERLING.card.expression(summonCreature("Spiderling", 3, 10, "spiderling", new DeckList(MOVE.card(), CREATE_ROCK.card())));
		VOODOO_HEX.card.expression(addCardToStack(FEAR.card(), new ActorsOnTilesQuery(new TilesInRadiusQuery(3), true)));
		DEBILITATING_FEAR.card.targetingInfo(new TargetingInfo(CARD_PLAYER,
				new RangeCondition(6),
				hasCardInZone(cardZone(new TargetQuery<>()), FEAR.card())));
	}

	private final GameCard card;

	FixedCards(GameCard card) {
		this.card = card;
	}
