...
public class CardStack extends CardZone<CardStackEntry> {
	...
	public void render(RenderingEnvironment re, InteractionState is, ConstraintPair screenPos) {
		Constraint padding = absolute(2).multiply(is.camera.zoom());
		Constraint iconSize = absolute(15).multiply(is.camera.zoom());
		...
		ConstraintBox box = new ConstraintBox(
				screenPos.x().add(absolute(TILE_RADIUS / 4).add(PADDING).multiply(is.camera.zoom())),
		...
		for (CardStackEntry entry : getCards()) {
			ConstraintBox iconBox = new ConstraintBox(
					...
					iconSize, iconSize);
			entry.icon().constraintBox(iconBox).render(re, is);
