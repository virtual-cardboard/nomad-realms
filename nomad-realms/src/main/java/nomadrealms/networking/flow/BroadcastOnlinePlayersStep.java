...
public class BroadcastOnlinePlayersStep implements NetworkStep {
			...
			public void update(FlowContext context) {
				for (Player player : context.onlinePlayers()) {
					List<Player> otherPlayers = context.onlinePlayers().stream()
							.filter(p -> !p.address().equals(player.address()))
							.collect(Collectors.toList());
					context.networkGraph().send(new OnlinePlayersListEvent(otherPlayers), player.address());
				}
				complete(true);
			}
		};
