...
public class TestParticle extends BaseParticle {
    ...
    public TestParticle(long lifetime, float x, float y, float vx, float vy, float ax, float ay) {
    ...
    public void render(RenderingEnvironment re, long timestamp) {
        float t = (timestamp - spawnTime) / 1000f;
        float currentX = x + vx * t + 0.5f * ax * t * t;
        float currentY = y + vy * t + 0.5f * ay * t * t;

		re.circleShaderProgram.use(re.glContext);
		re.circleShaderProgram.set("dimensions", new Vector2f(10, 10));
		re.circleShaderProgram.set("pos", new Vector2f(currentX, currentY));
		re.circleShaderProgram.set("color", new Vector4f(1, 1, 1, 1));
