...
public class MutableGameState {
	...
	public <T extends GameObject> T getCorresponding(T object) {
		...
		} else if (object instanceof GameCard) {
			return (T) cards.get(object.id());
		} else if (object instanceof Actor) {
			corresponding = (T) actor(object.id());
		} else {
			throw new RuntimeException("Could not get corresponding object for " + object);
		}
