	...
	public void render() {
		if (!done) {
			try {
				...
				HexagonShaderProgram hexagonSP = new HexagonShaderProgram(context().resourcePack().texturedTransformationVertexShader(), fragmentShader);
				loader().submit(new ShaderProgramLoadTask(hexagonSP, fragmentShader)).get();
