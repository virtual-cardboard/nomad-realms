...
public class ServerSyncedEventHandler implements SyncedEventHandler {
	...
	public void resolve(JoinGameSyncedEvent event, PacketAddress address) {
		System.out.println("Player " + event.name() + " joined from " + address);
		players.add(new Player(event.name(), address));
		PlayerListSyncedEvent playerListEvent = new PlayerListSyncedEvent(players);
		for (Player player : players) {
			networkNode.send(playerListEvent, player.address());
		}
	}
